...
BEGIN
	...
	BEGIN
		...
		SELECT 'Customer found' as item, 100 as pts
			...
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Customer MoreTechnology.com is NOT found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Customer MoreTechnology.com is found' as item, 0 as pts
			END
...
SELECT SUM(pts) FROM dbtester.dbo.TestScore
...
INSERT INTO tblMovie VALUES (4,'Dare or Die',110,'R')
 
INSERT INTO tblMovie VALUES (5,'EeeeGhads',88,'G')

INSERT INTO tblMovie VALUES (6,'Fire Shaft',75,'R')
...
---Code for Lab 6.3 Updating Data
/*Make sure you are in the dbMovie
...
DELETE tblMovie WHERE m_Rating = 'G'
...
CREATE PROC Chapter7ChallengeSelfChecker
AS
BEGIN
...
BEGIN
	...
	IF EXISTS(SELECT * FROM JProCo.Sys.Tables WHERE name = 'SalesInvoice')
	BEGIN
		INSERT INTO dbTester.dbo.TestScore
		SELECT 'SalesInvoice found' as item, 100 as pts

			IF EXISTS (SELECT * FROM JProCo.sys.columns SC INNER JOIN JProCo.sys.tables ST ON SC.[Object_id] = ST.[Object_id] WHERE SC.[name] = 'InvoiceID' AND ST.[name] = 'SalesInvoice' )
			
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceID field of dbo.SalesInvoice Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceID field of dbo.SalesInvoice Found' as item, 0 as pts
			END



			IF EXISTS (SELECT * FROM JProCo.sys.columns SC INNER JOIN JProCo.sys.tables ST ON SC.[Object_id] = ST.[Object_id] WHERE SC.[name] = 'OrderDate' AND ST.[name] = 'SalesInvoice' )
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'OrderDate field of dbo.SalesInvoice Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'OrderDate field of dbo.SalesInvoice Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'PaidDate'
						AND ST.[Name] = 'SalesInvoice')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'PaidDate field of dbo.SalesInvoice Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'PaidDate field of dbo.SalesInvoice NOT Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'CustomerID'
						AND ST.[Name] = 'SalesInvoice')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'CustomerID field of dbo.SalesInvoice Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'CustomerID field of dbo.SalesInvoice NOT Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'Comment'
						AND ST.[Name] = 'SalesInvoice')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Comment field of dbo.SalesInvoice Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Comment field of dbo.SalesInvoice NOT Found' as item, 0 as pts
			END

			IF ((SELECT Count(*) FROM JProCo.dbo.SalesInvoice)=1877)
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'SalesInvoice Populated correctly' as item, 150 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'SalesInvoice NOT Populated correctly' as item, 0 as pts
			END
	...
	BEGIN 
		INSERT INTO dbTester.dbo.TestScore
		SELECT 'Sales Invoice not found' as item, 0 as pts
	END


	IF EXISTS(SELECT * FROM JProCo.Sys.Tables WHERE name = 'SalesInvoiceDetail')
	BEGIN
		INSERT INTO dbTester.dbo.TestScore
		SELECT 'SalesInvoiceDetail found' as item, 100 as pts

			IF EXISTS (SELECT * FROM JProCo.sys.columns SC 
					INNER JOIN JProCo.sys.tables ST 
					ON SC.[Object_id] = ST.[Object_id] 
					WHERE SC.[name] = 'InvoiceDetailID' AND ST.[name] = 'SalesInvoiceDetail' )
			
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceDetailID field of dbo.SalesInvoiceDetail Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceDetailID field of dbo.SalesInvoiceDetail Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC 
					INNER JOIN JProCo.sys.tables ST 
					ON SC.[Object_id] = ST.[Object_id] 
					WHERE SC.[name] = 'InvoiceID' AND ST.[name] = 'SalesInvoiceDetail' )
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceID field of dbo.SalesInvoiceDetail Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'InvoiceID field of dbo.SalesInvoiceDetail Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'ProductID'
						AND ST.[Name] = 'SalesInvoiceDetail')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'ProductID field of dbo.SalesInvoiceDetail Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'ProductID field of dbo.SalesInvoiceDetail NOT Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'Quantity'
						AND ST.[Name] = 'SalesInvoiceDetail')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Quantity field of dbo.SalesInvoiceDetail Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'Quantity field of dbo.SalesInvoiceDetail NOT Found' as item, 0 as pts
			END
			...
			IF EXISTS (SELECT * FROM JProCo.sys.columns SC
						INNER JOIN JProCo.Sys.tables ST
						ON SC.Object_ID = ST.Object_id
						WHERE SC.[name] = 'UnitDiscount'
						AND ST.[Name] = 'SalesInvoiceDetail')
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'UnitDiscount field of dbo.SalesInvoiceDetail Found' as item, 50 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'UnitDiscount field of dbo.SalesInvoiceDetail NOT Found' as item, 0 as pts
			END

			IF ((SELECT Count(*) FROM JProCo.dbo.SalesInvoiceDetail)=6960)
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'SalesInvoiceDetail Populated correctly' as item, 150 as pts
			END
			ELSE
			BEGIN
				INSERT INTO dbTester.dbo.TestScore
				SELECT 'SalesInvoiceDetail NOT Populated correctly' as item, 0 as pts
			END
	...
	BEGIN 
		INSERT INTO dbTester.dbo.TestScore
		SELECT 'SalesInvoiceDetail not found' as item, 0 as pts
	END
...
-----End Chapter7Challege Tester
USE JProCo
GO

GO
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetNONWashingtonEmployees')
DROP PROC GetNONWashingtonEmployees
GO

CREATE PROCEDURE GetNONWashingtonEmployees 
AS
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetWashingtonEmployees')
DROP PROC GetWashingtonEmployees
GO
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetOvernightProducts')
DROP PROC GetOvernightProducts
GO

CREATE PROC GetOvernightProducts
AS
	SELECT * FROM dbo.CurrentProducts
	WHERE Category = 'Overnight-stay'
	GO


IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetMediumStayProducts')
DROP PROC GetMediumStayProducts
GO

CREATE PROC GetMediumStayProducts
AS
SELECT * FROM dbo.CurrentProducts
WHERE Category = 'Medium-Stay'
GO

IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetLongTermProducts')
DROP PROC GetLongTermProducts
GO
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetEmployeesFromSeattle')
DROP PROC GetEmployeesFromSeattle
GO
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetEmployeesByCity')
DROP PROC GetEmployeesByCity
GO
CREATE PROC GetEmployeesByCity @City VARCHAR(50)  = 'Seattle'
AS  
BEGIN  
 SELECT E.Firstname, E.Lastname,  
 L.City, L.[State]  
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetProductListByCategory')
DROP PROC GetProductListByCategory
GO
CREATE PROC GetProductListByCategory 
@Category VARCHAR(50)
AS
BEGIN
...
IF EXISTS(SELECT * FROM sys.Procedures WHERE [name] = 'GetGrantsByEmployee')
DROP PROC GetGrantsByEmployee 
GO
...
IF NOT EXISTS(SELECT * FROM sys.syslogins where name ='Murray')
CREATE LOGIN Murray
...
IF NOT EXISTS(SELECT * FROM sys.syslogins where name ='Sara')
CREATE LOGIN Sara
WITH PASSWORD = 'ABC$$123'
